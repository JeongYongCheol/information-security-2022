...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    text_size = len(text)
    split_text_num = int(text_size/8)
    iter_num = 0

    while(iter_num < split_text_num):
        result_encrypt += sdes(text[8*iter_num:8*(iter_num+1)], key, MODE_ENCRYPT)
        iter_num += 1

    return result_encrypt

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    result_decrypt = bitarray()
    ciphertext_size = len(ciphertext)
    split_text_num = int(ciphertext_size/8)
    iter_num = 0

    while(iter_num < split_text_num):
        result_decrypt += sdes(ciphertext[8*iter_num:8*(iter_num+1)], key, MODE_DECRYPT)
        iter_num += 1
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    ...
    while(iter_num < split_text_num):
        if iter_num == 0:
            exclusive_block = text[8*iter_num:8*(iter_num+1)]^iv
        else :
            exclusive_block = text[8*iter_num:8*(iter_num+1)]^result_encrypt[8*(iter_num-1):8*(iter_num)]

        result_encrypt += sdes(exclusive_block, key, MODE_ENCRYPT)
        iter_num += 1
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    ciphertext_size = len(ciphertext)
    split_text_num = int(ciphertext_size/8)
    iter_num = split_text_num-1

    while(iter_num > -1):
        exclusive_block = sdes(ciphertext[8*iter_num:8*(iter_num+1)], key, MODE_DECRYPT) 

        if iter_num > 0:
            result_decrypt = (exclusive_block^ciphertext[8*(iter_num-1):8*(iter_num)]) + result_decrypt
        else :
            result_decrypt = (exclusive_block^iv) + result_decrypt
...
random_iv = bitarray(bin(random.getrandbits(8)).replace('0b', '').zfill(8))
