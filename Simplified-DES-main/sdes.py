...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    for i in IP:
            permuted_text.append(text[i])

    permuted_text_left = permuted_text[0:4]
    permuted_text_right = permuted_text[4:8]
    round_keys = schedule_keys(key)
    
    if mode == 1 :
        permuted_text_left ^= round(permuted_text_right, round_keys[0])
        permuted_text_right ^= round(permuted_text_left, round_keys[1])
    else :
        permuted_text_left ^= round(permuted_text_right, round_keys[1])
        permuted_text_right ^= round(permuted_text_left, round_keys[0])

    permuted_text = permuted_text_right + permuted_text_left
    
    for i in IP_1:
            result.append(permuted_text[i])
